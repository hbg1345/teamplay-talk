...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "현재 공개 도구는 도메인 hub 중심이다. room_manage(action=create/join/switch/list/delete/restore/leave), "
        "form_manage(action=send/results/close), role_manage(action=start/finalize/set), "
        "roadmap_manage(action=build/view/decompose/member_tasks/digest), task_manage(action=add/update/delete), "
        ...
        "room_manage(action=create) 뒤에는 참여 안내 문구(invite_share_text)를 먼저 그대로 보여주고, 다음 단계는 팀원 초대/로드맵 생성/역할분배 중 무엇을 할지 질문으로 제안하라('로드맵을 만들까요, 역할 분배부터 할까요?'). "
        "기본 시작 흐름은 room_manage(create) → 팀원 초대 → roadmap_manage(build, 주제 기반 milestone) → role_manage(start/finalize/set) → roadmap_manage(decompose, 역할별 개인 todo) 순서로 안내하라. "
        ...
        "여러 태스크를 책임지는 역량/워크스트림 역할로 바꿔 role_manage(action=start)에 넣어라. "
