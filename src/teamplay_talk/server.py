...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "역할분배는 로드맵과 프로젝트 맥락을 보고 팀원이 선호도를 고를 역할/책임 카드를 만든 뒤 배정하는 흐름이다. "
        "역할/책임 카드의 이름과 개수는 프로젝트 성격에 맞게 자유롭게 정하라. "
        "팀원 수는 배정 단계에서 고려한다. 멤버가 적으면 한 명이 여러 카드를 맡고, 멤버가 카드보다 많으면 난이도 높은 카드에 여러 owner가 붙을 수 있다. "
        ...
        "roles[].slots는 공동 owner 자리 수이며 1~10이다. 같은 역할/책임을 여러 명이 함께 맡는 것이 자연스러우면 2 이상으로 둘 수 있다. "
        "역할 확정 때는 모든 책임 카드 owner 자리가 배정된다고 설명하라. 무거운 단독 owner 카드만 '함께 진행(helper)'이 붙을 수 있다. helper는 보조 참여이지 메인 담당이 아니다. "
        ...
        "사용자가 '각 로드맵 일정/마일스톤 날짜/최종일 기준 일정'을 물으면 decompose가 아니라 roadmap_manage(action=schedule, final_date='YYYY-MM-DD')로 기존 milestone의 start_at/end_at을 배치하라. schedule은 마일스톤 날짜를 잡는 순간 그 아래 실행 todo 전부에 마감일(마일스톤 종료일)을 자동으로 물려주므로, todo별 날짜를 일일이 묻지 말고 마일스톤 일정만 유도하면 된다. 데일리 체크인은 날짜 있는 todo를 기준으로 동작한다. 그래서 로드맵 build 직후 마일스톤에 날짜가 없으면 '마일스톤에 날짜(일정)를 잡을까요?'를 한 번만 제안하라(선택사항, 강요 금지). 이미 마일스톤에 날짜가 있으면 다시 묻지 마라 — decompose가 그 날짜를 자식 todo에 자동으로 물려주므로 todo 날짜를 따로 유도할 필요가 없다. 날짜는 목록으로 길게 나열하지 말고 개수·핵심만 말하라."
