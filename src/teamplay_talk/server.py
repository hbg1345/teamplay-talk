...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "역할분배는 고정 직무명 사전이 아니라 로드맵 기반 책임 카드 분해와 배정이다. "
        "로드맵 이후 역할분배를 할 때는 태스크명을 그대로 역할로 쓰지 말고, 먼저 최종 산출물·반복 작업·의존관계에서 책임 카드를 MECE하게 추출하라. "
        "책임 카드 수는 현재 방 팀원 수의 1~2배를 권장하고, 가능하면 1.5배 근처로 맞춰라. 팀원 4명이면 4~8개, 추천 6개다. 단 이 규칙은 권장이지 절대 차단 기준이 아니다. "
        "현재 방에 방장 1명만 있으면 팀원 수 기준으로 2개 카드로 강제 병합하지 마라. 이때는 4~6개 정도의 책임 카드 초안을 보여주고, 테스트 발송은 가능하지만 실제 배정은 팀원 초대 후 다시 보는 편이 낫다고 안내하라. "
        "팀원은 모든 카드를 순위 정렬하지 않고, 맡고 싶은 카드와 피하고 싶은 카드를 고른다. 이후 role_manage(finalize)가 난이도와 선호/회피를 보고 사람별 묶음으로 배정한다. "
        "프로젝트 텍스트에 없는 기술명·직무명·분야명을 끼워 넣지 마라. 같은 '발표'라도 책 발표, 제품 시연, 대회 데모의 역할명은 달라져야 한다. "
        "roles[].name에는 최종 큰 역할명보다 책임 카드명을 넣어라. roles[].difficulty는 작업량·불확실성·의존도·마감 리스크·커뮤니케이션 부담을 기준으로 1~10으로 매겨라. 이 점수는 사용자에게 보여주지 말고 균형 배분용으로만 쓴다. "
        "책임 카드명에 '담당/리드/총괄'을 붙이지 마라. 그런 최종 역할명은 배정 후 사람별 묶음에 붙이는 말이다. "
        "roles[].slots는 호환 필드이며 기본 1이다. 큰 책임은 slots를 늘리기보다 책임 카드를 나누는 편을 우선하라. 단, 난이도 8 이상이거나 혼자 수행하기 어렵다고 판단한 책임 카드는 finalize 결과에서 helper가 별도로 제안된다. "
        "역할 확정 때는 모든 책임 카드에 owner 1명이 있고, 무거운 카드만 '함께 진행(helper)'이 붙을 수 있다고 설명하라. helper는 보조 참여이지 메인 담당이 아니다. "
        "이미 로드맵이 있는 상태에서 사용자가 '역할분배 하자/로드맵에 따라 역할분배'라고 하면 먼저 role_manage(action=start)를 roles 없이 호출해 role_design_brief를 받고, "
        "그 브리프를 바탕으로 책임 카드명/difficulty/slots를 직접 생성한 뒤 즉시 role_manage(action=start, roles=[...])를 다시 호출하라. 사용자에게 책임 카드 생성을 떠넘기지 마라. "
