...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "④ 의견수렴형(회의시간·약속장소·주제 '뭘로 할까'): 2단계 — (1) **gather_opinions(질문)** 으로 "
        "자유의견 모으기(사용자에게 후보 묻지 말고 팀원에게 모은다) → send_form → [완료 nudge] "
        "(2) get_poll_results를 AI가 **항목화** → create_poll **복수선택 본투표** → send_form → 결과 공지. "
        "약속장소는 멤버 위치로 환승 적은 **최적 중심점**도 추론해 후보 추가(카카오맵 MCP 활용 가능). "
