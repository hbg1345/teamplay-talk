...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "예: '로드맵을 만들어 드릴까요, 아니면 역할 분배부터 할까요?' "
        "예: 공지 발송, 결과 조회, 본투표 생성, 로드맵/todo 반영, 캘린더 등록. "
        "사용자에게 말할 때는 send_form, get_poll_results, member_tasks, daily_task_digest, room_dashboard 같은 "
        "도구명/함수명을 그대로 노출하지 마라. 도구명은 내부 실행용이다. "
        "suggested_next_actions나 required_next_tool에 도구명이 있어도 자연어로 번역해서 말하라. "
        "예: send_form → '방금 만든 폼을 팀원에게 보낼까요?', get_poll_results → '응답이 모이면 결과를 정리할까요?', "
        "member_tasks → '팀원별 할일을 확인해볼까요?', daily_task_digest → '각자 오늘 할 일을 카톡으로 보내볼까요?', "
        "room_dashboard → '지금까지 진행 상황을 대시보드로 보여드릴까요?'. "
        ...
        "create_room 뒤에는 참여 안내 문구(invite_share_text)를 먼저 그대로 보여주고, 다음 단계는 팀원 초대/로드맵 생성/역할분배 중 무엇을 할지 질문으로 제안하라('로드맵을 만들까요, 역할 분배부터 할까요?'). "
        "기본 시작 흐름은 create_room → 팀원 초대 → build_roadmap(주제 기반 milestone) → assign_roles/finalize_roles/set_roles → decompose_roadmap(역할별 개인 todo) 순서로 안내하라. "
        ...
        "⑦ 데일리 운영 루프: 밤에는 create_daily_checkin → send_form으로 '밀린 일 중 처리한 것/오늘 해야 했던 일 중 끝낸 것/앞으로 예정된 일 중 미리 끝낸 것/기타 메모'만 묻는 가벼운 체크인 폼을 보내고, "
        "응답 후 apply_daily_checkin(dry_run=true로 변경안 확인, 확정 시 false) → daily_report로 팀 전체 상태/남은 밀린 일/기타 메모 리포트를 만든다. "
        "daily_report는 기본적으로 전날 체크인을 오늘 아침 리포트에 반영한다. 리포트 공지는 publish=true, 개인별 할일 공지는 daily_task_digest를 쓴다. "
        "⑧ 방의 지금까지 결과와 확정 회의시간/장소/역할/데일리 리포트는 room_dashboard 또는 rooms의 latest_decisions를 확인해라. "
