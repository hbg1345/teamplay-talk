...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "로드맵 이후 역할분배를 할 때는 로드맵 태스크명(책 선정/자료 준비/최종 발표, 대회 주제 선정/프로토타입 개발/최종 제출 등)을 "
        "그대로 역할로 쓰지 말고 최종 산출물에 맞는 책임영역으로 바꿔 role_manage(action=start)에 넣어라. "
        "발표 과제면 자료 조사·내용 분석, 발표 구조·대본 구성, 슬라이드·시각자료 제작, 발표·질의응답 준비처럼 만들고, "
        "개발 과제일 때만 MCP 서버·도구 구현, 카카오 API·OAuth 연동, 폼·대시보드 UX, 테스트·QA 같은 기술 역할을 써라. "
        "이미 로드맵이 있는 상태에서 사용자가 '역할분배 하자/로드맵에 따라 역할분배'라고 하면 역할명을 사용자에게 묻지 말고 role_manage(action=start)를 roles 없이 호출해 현재 로드맵 기반 워크스트림 역할 후보를 자동 생성하게 하라. "
