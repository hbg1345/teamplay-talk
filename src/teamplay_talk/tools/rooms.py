...
def _room_onboarding(room: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "next": "위 참여 안내 문구를 팀원에게 공유한 뒤, 먼저 주제로 로드맵을 만들고 그 마일스톤을 기준으로 역할분배와 todo 분해로 이어가세요.",
        ...
        "chat_response_hint": (
            ...
            "기본 흐름은 로드맵 먼저, 역할분배는 그 다음입니다. "
            "예: '먼저 프로젝트 주제로 로드맵을 잡아볼까요?' "
            "역할 먼저는 사용자가 역할이 이미 정해졌다고 명시할 때만 제안하세요. "
...
def register(mcp: FastMCP) -> None:
    ...
    async def rooms(invite_code: str | None = None) -> dict[str, Any]:
        ...
        초대 코드는 방장 전용 정보가 아니다. 방에 속한 멤버라면 역할(방장/멤버)과 무관하게
        누구나 자신이 속한 방의 초대 코드를 조회하고 팀원에게 공유할 수 있다. 방장이 아닌
        사용자가 물어봐도 주저하지 말고 그대로 알려줄 것.
        ...
        if invite_code:
            ...
            return {
                ...
                **_invite_payload(room),
                ...
                "next": "방 상세를 확인했습니다. 팀원을 더 초대하려면 참여 안내 문구를 공유하고, 로드맵이 없으면 주제 분석과 로드맵 생성부터 이어가세요.",
                "suggested_next_actions": [
                    "참여 안내 문구로 팀원 초대하기",
                    "로드맵이 없으면 로드맵 만들기",
                    "로드맵이 있으면 마일스톤 기준으로 역할분배 시작하기",
                    "진행 현황을 대시보드에서 확인하기",
                ],
                "chat_response_hint": (
                    "초대 코드는 사용자가 바로 복사할 수 있도록 백틱으로 감싼 인라인 코드(예: `초대코드`)로 표시하고, "
