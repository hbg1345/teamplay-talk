...
def _general_guide_payload(
    *,
    ...
    topic_notes = {
        ...
        "roles": "역할분배는 로드맵을 책임 카드로 나눈 뒤, 팀원이 고른 선호/회피와 난이도를 함께 봐서 사람별 역할 묶음으로 배정합니다.",
...
async def _guide_room(room_id: int | None, guide_topic: str | None = None) -> dict[str, Any]:
    ...
    topic_guides = {
        ...
        "roles": ["역할은 먼저 로드맵을 책임 카드로 나눈 뒤 배정합니다.", "팀원은 맡고 싶은 카드와 피하고 싶은 카드만 고르고, AI가 난이도와 연관도를 보고 사람별 묶음으로 정리합니다."],
...
def install(mcp: FastMCP) -> None:
    ...
    async def role_manage(
        ...
        """팀플톡 로드맵을 책임 카드로 나누고, 선호/회피와 난이도를 봐서 역할 배정안을 계산하거나 확정합니다."""
        target = {
            "start": "assign_roles",
