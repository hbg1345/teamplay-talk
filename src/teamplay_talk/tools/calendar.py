...
def register(mcp: FastMCP) -> None:
    ...
    async def calendar_create_room_event(
        ...
        missing = [
            {
                "nickname": m["nickname"],
                "error": "카카오 인증 토큰이 없습니다.",
                "required_action": "해당 팀원이 PlayMCP에서 teamplay-talk 카카오 권한을 다시 연결한 뒤 join_room 또는 rooms를 한 번 호출해야 합니다.",
            }
        ...
        return {
            ...
            "next": (
                "회의/공통 일정 등록이 끝났습니다. 캘린더 등록만으로 카톡 메시지가 보장되는 것은 아니므로, "
                "팀원이 바로 알아야 하면 별도 공지를 보내면 좋습니다."
            ),
            "suggested_next_actions": [
                "확정된 회의 시간을 팀원에게 카톡 공지하기",
                "회의 확정 기록을 대시보드에서 확인하기",
                "회의 준비물이 있으면 실행 todo 추가하기",
                "회의 전 개인별 준비 항목을 개인 공지하기",
            ],
            "chat_response_hint": (
                "캘린더 등록 성공/실패 멤버 수를 짧게 말하고, 다음 질문은 "
                "'팀원에게 확정 공지도 보낼까요?'처럼 공지 여부 확인으로 이어가세요. "
                "확정 공지는 todo로 만들지 말고 notify_room으로 보냅니다."
            ),
        }
        ...
        for task in storage.get_roadmap(room_id)["tasks"]:
            ...
            if user is None or not user.get("kakao_access_token"):
                failed.append({
                    ...
                    "required_action": "담당자가 PlayMCP에서 teamplay-talk 카카오 권한을 다시 연결한 뒤 join_room 또는 rooms를 한 번 호출해야 합니다.",
                })
                continue
