...
var CD_KO=[
 [{r:'user',t:'카카오 MCP 대회 로드맵 짜줘'},
  ...
  {r:'user',t:'좋아'},
  {r:'ai',t:'확정 — 각 단계를 <b>개인 todo</b>로 쪼개 배정했어요 (함봉구 백엔드 API · 이민지 폼 UX · 김주호 대시보드). 각자 카톡과 할 일 알림으로 보냈어요.'}],
 [{r:'user',t:'역할 좀 나눠줘'},
  {r:'ai',t:'로드맵을 <b>책임 카드</b>로 나눴어요. 맡고 싶은 것·피하고 싶은 것 받을까요?'},
  {r:'user',t:'ㅇㅇ'},
  {r:'ai',t:'팀원들 <b>선호·회피</b>를 받았어요. 이제 배정할게요.'},
  {r:'ai',t:'선호·회피·난이도 균형으로 배정했어요.<br>박세원 <b>기획 · QA</b> · 함봉구 <b>백엔드 API</b><br>이민지 <b>폼 UX · 데모 스토리</b> · 김주호 <b>대시보드</b><br>확정하고 공지했어요.'}],
 ...
 [{r:'user',t:'오늘 팀 현황 어때?'},
  {r:'ai',t:'어젯밤 체크인 요약이에요.<br>완료 4 · 진행 3 · 밀림 1<br>함봉구 백엔드 API <b>완료</b> · 이민지 폼 UX 60% · 김주호 대시보드 <b>밀림</b>(오늘 이월)<br>아침 리포트 보낼까요?'},
  {r:'user',t:'응'},
...
(function(){
  ...
  var STEPS_KO=[
   ...
   {cap:'로드맵 세우고 의견도 받아요', sub:'AI 마일스톤 초안 → 팀 검토 → 확정', member:'form', who:'이민지', channel:'카카오톡', think:'로드맵 설계 중', send:'검토 폼 전송',
    ...
    draft:'<b>6단계 로드맵</b> 초안이에요.<br>주제·기획 → 핵심 설계 → 개발·연동 → 데모·UX → 테스트·QA → 제출 <span>~7/10</span><br><span>팀원 검토 폼을 보낼게요</span>',
    kko:'<b>[팀플톡]</b> 로드맵 검토 요청<br><span>응답하기 &#9656;</span>',
    form:{type:'text', title:'로드맵 검토 · 의견', answer:'테스트 전에 보안 점검·유저 피드백 받는 단계도 넣어주세요'},
    result:'의견 반영 — <b>보안 점검·유저 피드백</b> 단계 추가.<br>7단계 로드맵으로 확정했어요.'},

   {cap:'역할을 나눠요', sub:'책임 카드 → 선호·회피 폼 → 균형 배정', member:'form', who:'이민지', channel:'카카오톡', think:'책임 카드 분해 중', send:'선호도 폼 전송',
   ...
   {cap:'할 일로 쪼개요', sub:'로드맵 단계 × 역할 → 개인별 todo → 일정', member:'notice', who:'이민지', channel:'카카오톡', big:true, think:'할 일 분해 중', send:'카톡 전달',
    ...
    draft:'로드맵 7단계를 <b>역할별 todo</b>로 쪼개 마감까지 붙였어요.<br>기획 → 박세원 · 백엔드 API → 함봉구 · 대시보드 → 김주호 · 폼 UX → 이민지',
...
_FEATURES = [
    ...
    ("roles", "역할 나누기", "로드맵을 책임 카드로 나누고, 선호·회피와 난이도를 함께 봐서 공평하게 묶어줘요."),
...
_PIPELINE = [
    ...
    ("03", "역할 분배", "책임 카드 선호·회피 → 난이도 균형 배정 → 방장 확인 후 확정.", "roles"),
...
var EN={
  ...
  "feat3t":"Split the roles","feat3d":"When who-does-what is fuzzy. Breaks the roadmap into responsibility cards, then weighs preference, avoidance, and difficulty for a fair split.",
  ...
  "pipe2t":"Role assignment","pipe2d":"Responsibility-card preference → difficulty-balanced assignment → owner confirms.",
...
var CD_EN=[
 [{r:"user",t:"Plan a roadmap for the Kakao MCP contest"},{r:"ai",t:"Mapped it in <b>6 stages</b> to the deadline (7/14).<br>1 Topic·plan <span>~6/20</span><br>2 Server + Kakao auth <span>~6/26</span><br>3 Core features <span>~7/4</span><br>4 Homepage·demo <span>~7/9</span><br>5 QC·rehearsal <span>~7/12</span><br>6 Submit <span>~7/14</span>"},{r:"user",t:"Nice"},{r:"ai",t:"Locked in — split each stage into <b>personal todos</b> (Bonggu Backend API · Minji Form UX · Juho Dashboard). Sent to each via KakaoTalk and to-do reminders."}],
 [{r:"user",t:"Split the roles"},{r:"ai",t:"Broke the roadmap into <b>responsibility cards</b>. Collect wants and avoids?"},{r:"user",t:"yep"},{r:"ai",t:"Got everyone's <b>wants and avoids</b>. Assigning now."},{r:"ai",t:"Balanced by preference·avoidance·difficulty.<br>Sewon <b>Planning · QA</b> · Bonggu <b>Backend API</b><br>Minji <b>Form UX · Demo story</b> · Juho <b>Dashboard</b><br>Confirmed and announced."}],
 ...
 [{r:"user",t:"How's the team doing today?"},{r:"ai",t:"Here's last night's check-in.<br>Done 4 · In progress 3 · Behind 1<br>Bonggu Backend API <b>done</b> · Minji Form UX 60% · Juho Dashboard <b>behind</b> (rolled to today)<br>Send the morning report?"},{r:"user",t:"yes"},{r:"ai",t:"Sent the <b>morning report</b> to team KakaoTalk. Behind items auto-carry to today's check-in."}]
];
var STEPS_EN=[
  ...
  <div class="wrap">
    ...
    <div class="makers">
      <div class="maker card rv d1 a">
        ...
        <div>
          <div class="mk-head"><h3>박세원</h3><div class="mk-links"><a class="mk-link" href="https://sewon-p.github.io" target="_blank" rel="noopener" aria-label="Portfolio"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9"><circle cx="12" cy="12" r="9"/><path d="M3 12h18M12 3a15 15 0 0 1 0 18M12 3a15 15 0 0 0 0 18"/></svg><span class="mk-txt">Portfolio</span></a><a class="mk-link" href="https://www.linkedin.com/in/sewon-park-328102368/" target="_blank" rel="noopener" aria-label="LinkedIn"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M6.94 5a2 2 0 1 1-4-.002 2 2 0 0 1 4 .002zM7 8.48H3V21h4V8.48zm6.32 0H9.34V21h3.94v-6.57c0-3.66 4.77-4 4.77 0V21H22v-7.93c0-6.17-7.06-5.94-8.72-2.91l.04-1.68z"/></svg><span class="mk-txt">LinkedIn</span></a></div></div>
          <ul class="cred">
